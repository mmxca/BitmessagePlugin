...
/**
 * Curl component based on the wrapper class for curl which was developed and implemented
...
class CurlComponent extends Component {
    ...
    public $components = ['BitmessagePlugin.CurlCache'];

    /**
     *  *  * @var bool */
    public $cacheIt = false;
    public $proxy = false;
    ...
    public function initialize(array $config = []) {
        ...
        $this->debug = (!(Configure::read('BitMessage.debug') == "0"));
        
        $this->options['CURLOPT_USERPWD'] = Configure::read('BitMessage.rpc_username') . ":" . Configure::read('BitMessage.rpc_password'); 
        $this->options['CURLOPT_HTTPAUTH'] = CURLAUTH_BASIC; 
                
        if (empty(Configure::read('BitMessage.cookie'))) {
            $this->cookie = 'phpmessage_cookie.txt';
        } else {
            $this->cookie = Configure::read('BitMessage.cookie');
        }

        $this->useCache = (!(Configure::read('BitMessage.useCache') == "0"));
    ...
    private function handle_response_headers($ch, $header) {
        ...
        if (strlen($header) > 2) {
            ...
            $key = rtrim($key, ':');
            if (!empty($this->responseArray[$key])) {
                if (is_array($this->responseArray[$key])) {
                    $this->responseArray[$key][] = $value;
                } else {
                    $tmp = $this->responseArray[$key];
                    $this->responseArray[$key] = array();
                    $this->responseArray[$key][] = $tmp;
                    $this->responseArray[$key][] = $value;
                }
            } else {
                $this->responseArray[$key] = $value;
